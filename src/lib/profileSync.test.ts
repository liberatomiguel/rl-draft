...
describe("mergeProfiles — never lose progress (#55.7)", () => {
  ...
  it("takes the MAX of every counter and peak — result regresses below neither side", () => {
    const a = make({ xp: 5000, wins: { easy: 5, normal: 1, hard: 0, legacy: 0 }, records: { bestOverall: { easy: 90, normal: 0, hard: 0, legacy: 0 }, bestOverallWorldwide: 90, bestOverallSam: 0 } });
    const b = make({ xp: 3000, wins: { easy: 1, normal: 4, hard: 2, legacy: 0 }, records: { bestOverall: { easy: 70, normal: 85, hard: 92, legacy: 0 }, bestOverallWorldwide: 92, bestOverallSam: 88 } });
